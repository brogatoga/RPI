...
void StudentTests() {
  TrainCar* engine = TrainCar::MakeEngine(); 
  TrainCar* sleeper = TrainCar::MakeSleepingCar();
  TrainCar* freight = NULL; 
  TrainCar* train = NULL; 

  for (int c = 0; c < 5; c++)
    PushBack(freight, TrainCar::MakeFreightCar(50+c*10));

  PrependCar(train, sleeper);
  PrependCar(train, engine); 

  while (freight != NULL) {
    AppendCar(train, freight);
  }

  std::cout << std::endl << "Student tests (PrependCar and AppendCar)" << std::endl;
  PrintTrain(train);
  DeleteAllCars(train);
}
