...
// add superhero to team
Team operator+(const Team &a, const Superhero &b) {
  Team team; 
  for (std::list<Superhero>::const_iterator it = a.superheroes.begin(); it != a.superheroes.end(); it++)
    team.superheroes.push_back(*it); 
  team.superheroes.push_back(b); 
  return team; 
}

// join two teams together
...
// Ostream << operator overload function
std::ostream& operator<<(std::ostream& ostr, const Superhero& s) {
...
int main() {
  ...
  assert(c > h);    // invisible beats speed
  assert(c < d);    // laser beats invisible
