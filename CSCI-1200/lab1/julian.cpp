...
int main()
{
  ...
  */
  ...
  /* Read input */
  for (int c = 0; c < 10; c++) {
    std::cout << std::endl << "#" << c+1 << ": Enter in month and day: "; 
    std::cin >> months[c] >> days[c]; 
  }


  /* Compute Julian day and output result */
  for (int c = 0; c < 10; c++) {
    julianDays[c] = julian_day(months[c], days[c], YEAR); 
    std::cout << julianDays[c] << std::endl;
  }
