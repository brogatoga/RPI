...
int main() {
  ...
  // TESTING REMOVE_MATCHING_ELEMENTS FUNCTION
  Vec<int> vt(10, 20); 
  vt[2] = 3;  vt[3] = 8; 
  vt[7] = 4;  vt[9] = 4; 

  cout << "Before calling remove_matching_elements: " << endl;
  vt.print(); 
  cout << "After calling it: " << endl;
  remove_matching_elements(vt, 20); 
  vt.print(); 


  // TESTING MASS PUSHBACKS
  cout << endl << endl << "Testing mass pushbacks: " << endl;
  Vec<int> vt2; 
  Vec<int>::size_type alloc = vt2.alloc(); 
  int counter = 0; 
  ...
  for (int c = 0; c < 5000000; c++) {
    ...
    if (vt2.alloc() != alloc) {
      alloc = vt2.alloc(); 
      counter++; 
      cout << "#" << counter << ": m_data was reallocated, new alloc size is now " << alloc << endl;
    }
  }
