...
template<class T> typename Vec<T>::size_type remove_matching_elements(Vec<T>& vec, const T& t) {
  typename Vec<T>::size_type removed = 0; 
  bool lastRemoved = false; 

  for (typename Vec<T>::iterator p = vec.begin(); p < vec.end(); p++) {
    if (lastRemoved) {
      p--;
      lastRemoved = false;
    }
...
template<class T> void Vec<T>::print() const {
  std::cout << "m_size = " << m_size << ", m_alloc = " << m_alloc << std::endl;
  for (typename Vec<T>::iterator p = m_data; p < m_data + m_size; p++) {
    std::cout << *p; 
    if (p != m_data+m_size-1) std::cout << ", "; 
  }
  std::cout << std::endl;
}
