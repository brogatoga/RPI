...
private:
    ...
    // CHECKPOINT 1 -- ITERATIVE VERSION -- O(log n)
    iterator find(const T& key_value, TreeNode<T>* p) {
        TreeNode<T>* q = p; 
        while (q != NULL) {
            if (q->value > key_value && q->left != NULL)
                q = q->left; 
            else if (q->value < key_value && q->right != NULL)
                q = q->right; 
            else break; 
        }
        return (q->value == key_value) ? iterator(q) : iterator(NULL);  
    }


    // RECURSIVE VERSION OF FIND -- O(log n)
    /*
    iterator find(const T& key_value, TreeNode<T>* p) {
