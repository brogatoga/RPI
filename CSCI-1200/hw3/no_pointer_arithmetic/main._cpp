...
void StudentTests() {
  ...
  //   * tables that store types other than char (DONE)
  //   * copy constructor (DONE)
  //   * assignment operator (DONE)
  //   * destructor (DONE)
  //   * invalid requests (comment out for final submission) (DONE)
  //
  // =======================================================  

  // Testing std::string (alternative data type, STL container in a Table) and the copy constructor
  Table<std::string> st(5, 5, "noob");
  Table<std::string> st2(st);
  st2.set(3, 3, "lolol"); 
  st.print();
  st2.print();
  std::cout << "[std::string]  test complete" << std::endl;

  // Testing double (another different data type, primitive type in a Table) and the assignment operator
  Table<double> dt(2, 10, 100.0);
  Table<double> dt2(1, 1, 0.0);
  dt2 = dt; 
  dt.set(1, 6, -55.7);
  dt.print(); 
  dt2.print();
  std::cout << "[double] test complete" << std::endl;

  // Testing the destructor (by manually calling the constructor then checking that the table is empty) 
  Table<char> ct(5, 5, 'A'); 
  ct.print(); 
  ct.~Table(); 
  ct.print(); 
  assert(ct.numRows() == 0); 
  assert(ct.numColumns() == 0); 
  std::cout << "Destructor test complete" << std::endl;

  // If rows = 1 and cols = 5, and pop_back_row is called, then the table should be empty
  Table<char> tsr(1, 5, 'B'); 
  tsr.pop_back_row();
  tsr.print(); 
  assert(tsr.numRows() == 0); 
  assert(tsr.numColumns() == 0); 
  std::cout << "Single row table test complete" << std::endl;

  // If rows = 5 and cols = 1, and pop_back_column is called, then the table should also be empty
  Table<char> tsc(5, 1, 'C'); 
  tsc.pop_back_column(); 
  tsc.print(); 
  assert(tsc.numRows() == 0); 
  assert(tsc.numColumns() == 0); 
  std::cout << "Single column table test complete" << std::endl;

  // Invalid requests (commented out the invalid lines that cause an error within the program)
  Table<char> tinvalid(5, 5, 'T'); 
  Table<char> tinvalid2(1, 1, 'P'); 
  //Table<char> tinvalid3(-90, 50, 'A'); 
  //tinvalid.set(6, 6, 'A'); 
  //char tchar = tinvalid.get(-1, 5);
  //tinvalid2.pop_back_row(); tinvalid2.pop_back_row();   
  //tinvalid2.pop_back_column(); tinvalid2.pop_back_column(); 
  //tinvalid.push_back_row(std::vector<char>(6, 'f')); 
  //tinvalid.push_back_column(std::vector<char>(7, 'a')); 
}
